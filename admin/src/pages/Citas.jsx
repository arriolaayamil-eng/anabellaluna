...
import { FaCalendarPlus, FaSync, FaClock, FaUsers, FaMapMarkerAlt, FaPhoneAlt, FaBell, FaCheckCircle, FaTimes, FaSave, FaList, FaGripVertical, FaPlus, FaEdit, FaTrash, FaChartLine, FaArrowUp, FaArrowDown, FaPercentage, FaCalendarAlt, FaTasks, FaExclamationCircle } from 'react-icons/fa';
    ...
    <div className={`min-h-screen px-6 lg:px-8 pt-4 pb-6 ${isDark ? 'bg-main-dark-bg' : 'bg-gray-50'}`}>
      ...
      <div className="flex items-center justify-between flex-wrap gap-3 mb-6">
        ...
        {activeSection === 'agenda' && (
          <button onClick={() => setShowModalCita(true)} className="flex items-center gap-2 px-5 py-2.5 rounded-xl text-white text-sm font-medium bg-blue-500 hover:bg-blue-600 transition-all shadow-sm hover:shadow-md">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
          <div className={`${currentMode === 'Dark' ? 'bg-gray-900' : 'bg-white'} rounded-2xl shadow-2xl max-w-lg w-full max-h-[90vh] overflow-hidden flex flex-col`}>
            ...
            <div className="flex-1 overflow-y-auto p-6">
              ...
              <div className="text-center">
                <button onClick={() => { setShowModalPendientes(false); setActiveSection('tareas'); }} className="px-6 py-3 bg-indigo-500 text-white rounded-lg hover:bg-indigo-600 transition-colors font-medium flex items-center gap-2 mx-auto">
                  <FaTasks /> Ver Gesti&oacute;n de Tareas
                </button>
              </div>
            </div>
          </div>
