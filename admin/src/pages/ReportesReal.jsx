...
const ReportesReal = () => {
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const [configRes, historyRes, receivedRes] = await Promise.all([
        crmService.reports.getConfig(),
        crmService.reports.getHistory(),
        crmService.reports.getReceived().catch(() => []),
      ]);

      setReportConfig(configRes || null);
      setHistory(Array.isArray(historyRes) ? historyRes : []);
      setReceivedReports(Array.isArray(receivedRes) ? receivedRes : []);

      if (configRes?.annualReportSelections) {
        setSelectedReports(configRes.annualReportSelections);
      } else {
        const defaultSelections = {};
        REPORT_DEFINITIONS.forEach((r) => {
          defaultSelections[r.id] = true;
        });
      ...
      const dataPromises = REPORT_DEFINITIONS.map(async (report) => {
        try {
          ...
          return { id: report.id, data: { error: e?.message || 'Error al cargar datos' } };
        }
      });
  ...
  const generateReport = async (type = 'manual') => {
    ...
    try {
      ...
      // Let Syncfusion charts finish any pending animation frames
      await new Promise((r) => setTimeout(r, 500));

      const currentUser = authService.getCurrentUser();
      const { blob, period } = await exportDashboardToPDF({
        kpiContainer: kpiRef.current,
        chartElements,
        ...
        month: selectedMonth,
        type,
        userName: currentUser?.nombre || currentUser?.username || 'Administrador',
      ...
      a.href = url;
      a.download = `reporte-ejecutivo-${period}.pdf`;
      ...
      try {
        ...
        // eslint-disable-next-line no-console
        console.warn('PDF downloaded but history registration failed:', histErr);
      }
  ...
  const cardBase = 'rounded-xl shadow-md p-6 bg-gray-50 dark:bg-gray-800/60 border border-gray-100 dark:border-gray-700 transition-all duration-300 hover:shadow-lg';
  const cardHover = `${cardBase} hover:scale-[1.01] cursor-pointer`;
    ...
    return (
      <div className={`min-h-screen px-6 lg:px-8 pt-4 pb-6 ${currentMode === 'Dark' ? 'bg-main-dark-bg' : 'bg-gray-50'}`}>
        <div className="mb-6">
          ...
          <p className={`text-sm mt-1 ${currentMode === 'Dark' ? 'text-gray-400' : 'text-gray-500'}`}>Analítica avanzada del negocio</p>
        </div>
  ...
  return (
    <div className={`min-h-screen px-6 lg:px-8 pt-4 pb-6 relative ${currentMode === 'Dark' ? 'bg-main-dark-bg' : 'bg-gray-50'}`}>
      {exportProgress !== null && (
      ...
      <div className="flex flex-wrap gap-2 mb-6 border-b dark:border-gray-700 pb-4">
        {[
          ...
          { id: 'received', label: 'Reportes Recibidos', icon: <FaInbox /> },
      ...
      {activeTab === 'selection' && (
        <div className={cardBase}>
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-lg font-semibold dark:text-gray-100">Selecciona los reportes para incluir en el informe anual</h3>
            ...
            {REPORT_DEFINITIONS.map((report) => (
              <label
                ...
                className={`flex items-center gap-3 p-4 border rounded-lg cursor-pointer transition-all ${
                  selectedReports[report.id] ? 'border-2 bg-blue-50 dark:bg-blue-900/20' : 'border-gray-200 dark:border-gray-700 hover:border-gray-300'
                }`}
              ...
              >
                <input
                  ...
                  style={{ accentColor: currentColor }}
                />
                <div className="text-xl" style={{ color: report.color }}>
                  {report.icon}
                </div>
            ...
            <div className="space-y-6">
              ...
              <div className="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
                <p className="text-sm text-blue-700 dark:text-blue-300">
                  <strong>ℹ️ Información:</strong> Los reportes mensuales se generan automáticamente el día {reportConfig?.autoSendDay || 1} de cada mes y se envían al ERP en formato PDF. También puedes generar reportes manualmente en cualquier momento.
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="space-y-2 max-h-96 overflow-y-auto">
              {REPORT_DEFINITIONS.map((report) => (
                <div key={report.id} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                  <div className="flex items-center gap-3">
                    <div className="text-lg" style={{ color: report.color }}>
                      {report.icon}
                    </div>
                    <span className="dark:text-gray-200 text-sm">{report.name}</span>
                  </div>
                  {selectedReports[report.id] ? <FaCheck className="text-green-500" /> : <span className="text-xs text-gray-400">No incluido</span>}
                </div>
              ))}
            </div>
          </div>
          ...
          {history.length === 0 ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody>
                  {history.map((report) => (
                    <tr key={report._id} className="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800">
                      ...
                      <td className="p-3 dark:text-gray-300 text-sm">{new Date(report.generatedAt).toLocaleString('es')}</td>
                      ...
                      <td className="p-3 text-center">
                        ...
                        {report.sentToERP && report.sentAt && (
                          <span className="text-xs text-gray-500">{new Date(report.sentAt).toLocaleDateString('es')}</span>
                        )}
                      </td>
                    </tr>
                  ))}
      ...
      {activeTab === 'received' && (
        <div className={cardBase}>
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-lg font-semibold dark:text-gray-100">📥 Reportes Recibidos del CRM</h3>
            <button
              onClick={async () => {
                try {
                  const res = await crmService.reports.getReceived();
                  setReceivedReports(res || []);
                } catch (err) {
                  console.error('Error loading received reports:', err);
                }
              }}
              className="flex items-center gap-2 px-4 py-2 bg-gray-100 dark:bg-gray-800 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-700 dark:text-gray-200 text-sm"
            >
              <FaSync /> Actualizar
            </button>
          </div>
          ...
          {receivedReports.length === 0 ? (
            <div className="text-center py-12 text-gray-500">
              <FaInbox className="text-6xl mx-auto mb-4 opacity-30" />
              <p>No hay reportes recibidos del CRM aún</p>
              <p className="text-xs text-gray-400 mt-2">Los agentes pueden enviar reportes desde el CRM</p>
            </div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-gray-100 dark:bg-gray-800">
                  <tr>
                    <th className="p-3 text-left dark:text-gray-300">Agente</th>
                    <th className="p-3 text-left dark:text-gray-300">Tipo</th>
                    <th className="p-3 text-left dark:text-gray-300">Período</th>
                    <th className="p-3 text-left dark:text-gray-300">Recibido</th>
                    <th className="p-3 text-right dark:text-gray-300">Tamaño</th>
                    <th className="p-3 text-center dark:text-gray-300">Acciones</th>
                  </tr>
                </thead>
                <tbody>
                  {receivedReports.map((rpt) => (
                    <tr key={rpt._id} className="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800">
                      <td className="p-3 dark:text-gray-300 font-medium">{rpt.agentName || 'Agente'}</td>
                      <td className="p-3 dark:text-gray-300">
                        <span
                          className={`px-2 py-1 rounded text-xs font-medium ${
                            rpt.type === 'annual'
                        ...
                        >
                          {rpt.type === 'annual' ? 'Anual' : rpt.type === 'monthly' ? 'Mensual' : 'Manual'}
                        </span>
                      </td>
                      <td className="p-3 dark:text-gray-300">{rpt.period || '-'}</td>
                      <td className="p-3 dark:text-gray-300 text-sm">{new Date(rpt.createdAt).toLocaleString('es')}</td>
                      <td className="p-3 dark:text-gray-300 text-right text-sm">
                        {rpt.fileSize ? `${(rpt.fileSize / 1024).toFixed(0)} KB` : '-'}
                      </td>
                      <td className="p-3 text-center">
                        <button
                          onClick={async () => {
                            try {
                              ...
                              const url = `${API_CONFIG.baseURL}/crm/reports/received/${rpt._id}/download`;
                              ...
                              link.download = rpt.filename || `reporte-${rpt.period || 'crm'}.pdf`;
                          ...
                          className="flex items-center gap-1 px-3 py-1.5 text-sm text-white rounded-lg hover:opacity-90 mx-auto"
                          style={{ backgroundColor: currentColor }}
                        >
