...
function writeCachedMessages(conversationId, messages) {
  ...
  try {
    ...
    // localStorage can be full or unavailable; chat must keep working.
  }
}
...
export function useAIChat(conversationId) {
  ...
  const sendMessage = useCallback(async (text) => {
    ...
    if (!cleanText || !conversationIdRef.current || sendingRef.current) return null;
    ...
    try {
      const result = await aiService.sendMessage(conversationIdRef.current, cleanText, clientMessageId);
      ...
      const nextMessages = mergeMessages(fresh || []);
      setMessages(nextMessages);
