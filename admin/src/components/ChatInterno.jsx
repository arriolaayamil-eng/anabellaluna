...
const ChatInterno = () => {
      ...
      const enquiries = (Array.isArray(items) ? items : [])
        ...
        .map(item => {
          ...
          return {
            ...
            ultimoMensaje: item.notes || (item.type === 'visit_scheduled' ? 'Solicitud de visita' : 'Consulta web'),
            hora: formatTimeLabel(createdAt),
            noLeidos: meta.read ? 0 : 1,
            online: false,
  ...
  const getDisplayedConversations = () => {
    let items = [];
    if (activeTab === 'all') items = [...conversaciones, ...agentChats];
    else if (activeTab === 'agents') items = agentChats;
    else items = conversaciones;
  ...
  const totalAgentNoLeidos = agentChats.reduce((sum, c) => sum + c.noLeidos, 0);
  const totalAllNoLeidos = totalNoLeidos + totalAgentNoLeidos;

  const loadChatHistory = async (partnerId) => {
    try {
      const messages = await chatService.getHistory(partnerId, { limit: 50 });
  ...
  const handleEnviarMensaje = async () => {
    if (mensajeNuevo.trim() && chatActivo) {
      if (chatActivo.source === 'agent' && chatActivo.partnerId) {
        try {
          await chatService.send(chatActivo.partnerId, mensajeNuevo.trim(), { receiverType: 'agent' });
          setMensajeNuevo('');
  ...
  const handleOpenChat = async (conv) => {
    setChatActivo(conv);
    if (conv.noLeidos > 0) markAsRead(conv);
    if (conv.source === 'agent' && conv.partnerId) {
  ...
  return (
    <div className="nav-item absolute right-5 md:right-40 top-16 bg-white dark:bg-[#42464D] rounded-lg w-[420px] shadow-xl z-50">
      ...
      <div className="flex border-b dark:border-gray-600">
        {[
          { key: 'all', label: 'Todos', badge: totalAllNoLeidos },
          { key: 'agents', label: 'Agentes', badge: totalAgentNoLeidos, icon: <FaUsers className="text-xs" /> },
          { key: 'web', label: 'Web', badge: totalNoLeidos, icon: <FaGlobe className="text-xs" /> },
        ].map(tab => (
          <button key={tab.key} type="button" onClick={() => setActiveTab(tab.key)}
            className={`flex-1 py-2 px-3 text-sm font-medium transition-colors flex items-center justify-center gap-1 ${
              activeTab === tab.key ? 'border-b-2' : 'text-gray-500 hover:text-gray-700 dark:text-gray-400'
            }`}
            style={activeTab === tab.key ? { borderColor: currentColor, color: currentColor } : {}}>
            {tab.icon} {tab.label}
            {tab.badge > 0 && (
      ...
      <div className="p-3 border-b dark:border-gray-600">
        <div className="relative">
          ...
          <input type="text" placeholder="Buscar por nombre, email..." value={searchTerm}
      ...
      {!chatActivo ? (
        <div className="max-h-80 overflow-y-auto">
          ...
          {filteredConversaciones.map((conv) => (
            <div key={`${conv.source}-${conv.id}`} onClick={() => handleOpenChat(conv)}
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-1">
                    <h4 className="font-bold text-sm dark:text-gray-200 truncate">{conv.nombre}</h4>
                    <span className="text-xs text-gray-500 dark:text-gray-400 flex-shrink-0 ml-2">{conv.hora}</span>
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    <p className="text-xs text-gray-600 dark:text-gray-400 truncate flex-1">{conv.ultimoMensaje}</p>
                    {conv.noLeidos > 0 && (
        ...
        <div className="flex flex-col h-[450px]">
          <div className="p-3 border-b dark:border-gray-600 flex items-center gap-3">
            <button type="button" onClick={() => { setChatActivo(null); setChatMessages([]); }}
              className="text-gray-600 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-200 text-lg">←</button>
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400 flex items-center gap-1">
                {chatActivo.source === 'agent' && chatActivo.online && <FaCircle className="text-green-500 text-[8px]" />}
                {chatActivo.rol}
              </p>
            </div>
          </div>
          ...
          {chatActivo.source === 'agent' ? (
            <>
                    ...
                    return (
                      <div key={msg._id || idx} className={`flex ${isOwn ? 'justify-end' : 'justify-start'}`}>
                        <div className={`max-w-[75%] px-3 py-2 rounded-lg text-sm ${
                          isOwn ? 'bg-blue-500 text-white rounded-br-none' : 'bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-200 rounded-bl-none shadow-sm'
                        }`}>
                          <p>{msg.content}</p>
              ...
              <div className="p-3 border-t dark:border-gray-600">
                <div className="flex items-center gap-2">
                  <input type="text" value={mensajeNuevo} onChange={(e) => setMensajeNuevo(e.target.value)}
                    onKeyPress={(e) => e.key === 'Enter' && handleEnviarMensaje()} placeholder="Escribe un mensaje..."
                  ...
                  <button type="button" onClick={handleEnviarMensaje} className="p-2 rounded-lg text-white transition-colors"
                    style={{ backgroundColor: currentColor }} title="Enviar"><FaPaperPlane /></button>
                </div>
              </div>
            </>
          ) : (
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50 dark:bg-gray-800">
                ...
                <div className="bg-white dark:bg-gray-700 rounded-lg p-4 shadow-sm">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {chatActivo.email && (
                      <a href={`mailto:${chatActivo.email}?subject=Re: Consulta sobre ${chatActivo.propiedad || 'propiedad'}`}
                    ...
                    {chatActivo.phone && (
                      <a href={`https://wa.me/${chatActivo.phone.replace(/\D/g, '')}`} target="_blank" rel="noopener noreferrer"
