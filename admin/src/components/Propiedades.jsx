...
const Propiedades = () => {
  ...
  const getTipoIcon = (tipo) => {
    const icons = { departamento: '🏢', casa: '🏠', ph: '🏘️', local: '🏪', oficina: '🏛️', terreno: '🌳', cochera: '🚗' };
  ...
  const loadPropiedades = useCallback(async () => {
    ...
    try {
      ...
      const propiedadesData = (Array.isArray(items) ? items : [])
        ...
        .map(p => ({
          id: p._id,
          nombre: p.titulo || p.title || p.nombre || 'Sin título',
          direccion: p.direccion || p.address || p.ubicacion?.direccion || 'Sin dirección',
          precio: formatPrice(p.precio || p.price, p.moneda || p.currency),
          estado: p.estado || p.status || 'Disponible',
          visitas: p.metadata?.visitas || p.visitas || 0,
          tipo: p.tipo || p.type || 'casa',
      ...
      setStats({
        ...
        disponibles: summary?.propiedades?.disponibles || propiedadesData.filter(p => p.estado?.toLowerCase() === 'disponible').length,
      });
    } catch (err) {
