...
const Tareas = () => {
  ...
  const getItemIcon = (tipo, source) => {
    ...
    const icons = { llamada: '📞', reunion: '👥', seguimiento: '🔄', documentacion: '📄', visita: '🏠', otro: '📋' };
  ...
  const formatVencimiento = (fecha) => {
    ...
    if (itemDate.getTime() === today.getTime()) return `Hoy ${d.toLocaleTimeString('es-AR', { hour: '2-digit', minute: '2-digit' })}`;
    if (itemDate.getTime() === tomorrow.getTime()) return `Mañana ${d.toLocaleTimeString('es-AR', { hour: '2-digit', minute: '2-digit' })}`;
    if (itemDate < today) return `Vencido - ${d.toLocaleDateString('es-AR', { day: 'numeric', month: 'short' })}`;
  ...
  const loadTareas = useCallback(async () => {
    ...
    try {
      ...
      (Array.isArray(tareas) ? tareas : [])
        .filter(t => t.kanbanColumn !== 'done' && t.status !== 'Close')
        .forEach(t => {
          allItems.push({
            id: t._id,
            titulo: t.title || t.titulo || t.nombre || 'Sin título',
            descripcion: t.summary || t.descripcion || '',
            prioridad: t.priority || t.prioridad || 'media',
            vencimiento: t.dueDate || t.fechaVencimiento,
            completada: false,
            source: 'tarea',
            tipo: t.tipo,
            agente: t.agente || '',
      ...
      (Array.isArray(citas) ? citas : [])
        ...
        .forEach(c => {
          allItems.push({
            ...
            descripcion: c.notas || c.ubicacion || '',
            prioridad: 'alta',
            vencimiento: c.fecha,
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {item.source === 'cita' && <FaCalendarAlt className="text-blue-500 text-xs" title="Cita" />}
                        {item.source === 'visit' && <FaHome className="text-green-500 text-xs" title="Solicitud web" />}
                      </div>
                    </div>
                    {item.agente && (
