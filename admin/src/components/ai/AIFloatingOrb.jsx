...
const FloatingChat = ({ conversationId, onClose, isDark }) => {
  ...
  const handleApprove = async (id) => { setApproving(true); await approveTool(id); setApproving(false); };
  const handleReject  = async (id, reason) => { setApproving(true); await rejectTool(id, reason); setApproving(false); };
          ...
          {messages.length === 0 && !loading && !hydrating && (
            <div style={{
              ...
              <div style={{ fontSize: 12, lineHeight: 1.5, maxWidth: 260 }}>
                Podés pedirme agendar citas, consultar reportes, analizar campañas o pedir recomendaciones.
              </div>
        ...
        <div style={{
          ...
          <button
            ...
            style={{
              ...
              borderRadius: 10,
              border:       'none',
              background:   loading || !input.trim() ? (isDark ? '#334155' : '#e2e8f0') : 'linear-gradient(135deg, #6b3de8, #9b6dff)',
...
const AIFloatingOrb = () => {
  ...
  return (
    <>
      ...
      <OrbButton onClick={handleToggle} isOpen={isOpen} pulse={!conversationId} />
