...
const MiPerfil = () => {
  ...
  return (
    ...
    <div className={`min-h-screen px-6 lg:px-8 pt-4 pb-6 ${currentMode === 'Dark' ? 'bg-main-dark-bg' : 'bg-gray-50'}`}>
      ...
      {/* ── Seguridad / 2FA ── */}
      <div className="mt-8">
        <h3 className={`text-lg font-semibold mb-4 flex items-center gap-2 ${currentMode === 'Dark' ? 'text-gray-100' : 'text-gray-800'}`}>
        ...
        {tfaLoading ? (
          ...
          <div className="space-y-4">
            {!setupStep && !showDisable && !showRegen && (
              <>
                <div className={`p-5 rounded-2xl border ${currentMode === 'Dark' ? 'border-gray-700/50' : ''}`} style={{ borderColor: tfaStatus?.enabled ? '#22c55e33' : '#f59e0b33', background: tfaStatus?.enabled ? '#f0fdf408' : '#fffbeb08' }}>
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1">
                      <p className={`font-semibold text-sm ${currentMode === 'Dark' ? 'text-white' : 'text-gray-800'}`}>Autenticación de dos factores (2FA)</p>
                ...
                {!tfaStatus?.enabled && (
                  <button onClick={handleInitSetup} disabled={setupLoading} className={`flex items-center gap-3 w-full p-4 rounded-2xl border transition-colors ${currentMode === 'Dark' ? 'border-gray-700/50 hover:bg-gray-800/50' : 'border-gray-200 hover:bg-gray-50'}`}>
                    ...
                    <div className="text-left flex-1">
                      <p className={`font-semibold text-sm ${currentMode === 'Dark' ? 'text-white' : 'text-gray-800'}`}>Activar 2FA</p>
                ...
                {tfaStatus?.enabled && (
                  <>
                    <button onClick={() => setShowRegen(true)} className={`flex items-center gap-3 w-full p-4 rounded-2xl border transition-colors ${currentMode === 'Dark' ? 'border-gray-700/50 hover:bg-gray-800/50' : 'border-gray-200 hover:bg-gray-50'}`}>
                      ...
                      <div className="text-left flex-1">
                        <p className={`font-semibold text-sm ${currentMode === 'Dark' ? 'text-white' : 'text-gray-800'}`}>Regenerar códigos de recuperación</p>
                        <p className="text-xs text-gray-500 dark:text-gray-400">{tfaStatus.recoveryCodesRemaining} código(s) restante(s)</p>
                      </div>
                    </button>
                    <button onClick={() => setShowDisable(true)} className="flex items-center gap-3 w-full p-4 rounded-2xl border border-red-200 dark:border-red-500/30 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors">
            ...
            {/* Setup: QR */}
            {setupStep === 'qr' && qrData && (
              <div className={`p-5 rounded-2xl border ${currentMode === 'Dark' ? 'border-gray-700/50 bg-secondary-dark-bg' : 'border-gray-200 bg-white shadow-sm'}`}>
                <div className="text-center mb-4">
                  <FiShield size={28} className="mx-auto mb-2" style={{ color: currentColor }} />
                  <h4 className={`font-bold ${currentMode === 'Dark' ? 'text-white' : 'text-gray-800'}`}>Escaneá el código QR</h4>
                ...
                <form onSubmit={handleVerifySetup} className="space-y-3">
                  <input ref={setupCodeRef} type="text" inputMode="numeric" maxLength={6} value={setupCode} onChange={(e) => { setSetupCode(e.target.value); setSetupError(''); }} className="w-full px-4 py-3 rounded-xl border border-gray-200 dark:border-gray-700/70 text-center text-2xl font-mono tracking-[0.3em] focus:outline-none focus:ring-2 focus:ring-sky-500 dark:bg-gray-900/40 dark:text-gray-100" placeholder="000000" autoFocus required />
            ...
            {setupStep === 'codes' && recoveryCodes.length > 0 && (
              <div className="p-5 rounded-2xl border border-amber-200 dark:border-amber-500/30 bg-amber-50/50 dark:bg-amber-900/10">
                <div className="text-center mb-4">
                  <FiLock size={28} className="mx-auto mb-2 text-amber-600 dark:text-amber-400" />
                  <h4 className={`font-bold ${currentMode === 'Dark' ? 'text-white' : 'text-gray-800'}`}>Códigos de recuperación</h4>
            ...
            {/* Disable 2FA */}
            {showDisable && (
              <div className={`p-5 rounded-2xl border border-red-200 dark:border-red-500/30 ${currentMode === 'Dark' ? 'bg-secondary-dark-bg' : 'bg-white shadow-sm'}`}>
            ...
            {showRegen && (
              <div className={`p-5 rounded-2xl border border-blue-200 dark:border-blue-500/30 ${currentMode === 'Dark' ? 'bg-secondary-dark-bg' : 'bg-white shadow-sm'}`}>
                <h4 className={`font-bold mb-3 flex items-center gap-2 ${currentMode === 'Dark' ? 'text-white' : 'text-gray-800'}`}><FiRefreshCw size={18} style={{ color: currentColor }} /> Regenerar códigos de recuperación</h4>
                ...
                <form onSubmit={handleRegenCodes} className="space-y-3">
                  <input type="password" value={regenPassword} onChange={(e) => { setRegenPassword(e.target.value); setRegenError(''); }} className="w-full px-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700/70 focus:outline-none focus:ring-2 focus:ring-sky-500 dark:bg-gray-900/40 dark:text-gray-100" placeholder="Contraseña actual" required />
                  <input type="text" inputMode="numeric" maxLength={6} value={regenCode} onChange={(e) => { setRegenCode(e.target.value); setRegenError(''); }} className="w-full px-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700/70 text-center font-mono tracking-wider focus:outline-none focus:ring-2 focus:ring-sky-500 dark:bg-gray-900/40 dark:text-gray-100" placeholder="Código TOTP (6 dígitos)" required />
