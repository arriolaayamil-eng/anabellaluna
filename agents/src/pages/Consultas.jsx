...
import { FaEnvelope, FaCalendarCheck, FaPhone, FaHome, FaSync, FaUser, FaClock, FaFilter, FaSearch, FaComments, FaPaperPlane, FaCircle, FaCheck, FaCheckDouble, FaUserCircle, FaBuilding, FaArrowLeft } from 'react-icons/fa';
...
const Consultas = () => {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      crmService.rewards.checkMilestones('enquiry').catch(() => {});
      ...
      {activeTab === 'chat' && (
        <div className={`rounded-2xl overflow-hidden border ${currentMode === 'Dark' ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} shadow-lg`} style={{ height: 'calc(100vh - 280px)', minHeight: '500px' }}>
          <div className="flex h-full">
            ...
            <div className={`${selectedChat ? 'hidden md:flex' : 'flex'} w-full md:w-80 flex-shrink-0 md:border-r ${currentMode === 'Dark' ? 'border-gray-700 bg-gray-900' : 'border-gray-200 bg-gray-50'} flex-col`}>
                ...
                {conversations.filter(c => !c.partner?.isErp).length > 0 && (
                  <div className="p-2">
                    ...
                    {conversations.filter(c => !c.partner?.isErp).map((conv) => (
                      ...
                      >
                        <div className={`relative w-12 h-12 rounded-full flex-shrink-0 flex items-center justify-center font-bold text-white ${
            ...
            {/* Chat Area */}
            {/* Mobile: hidden when no chat selected. Desktop: always visible */}
            <div className={`${selectedChat ? 'flex' : 'hidden md:flex'} flex-1 flex-col w-full md:w-auto`}>
              {selectedChat ? (
                <>
                  {/* Chat Header */}
                  <div className={`p-3 md:p-4 border-b ${currentMode === 'Dark' ? 'border-gray-700 bg-gray-800' : 'border-gray-200 bg-white'} flex items-center gap-3`}>
                    {/* Back button - only visible on mobile */}
                    <button
                      onClick={() => setSelectedChat(null)}
                      className={`md:hidden p-2 -ml-1 rounded-lg transition-colors ${currentMode === 'Dark' ? 'hover:bg-gray-700 text-gray-300' : 'hover:bg-gray-100 text-gray-600'}`}
                    >
                      <FaArrowLeft />
                    </button>
                    <div className="w-10 h-10 md:w-12 md:h-12 rounded-full flex-shrink-0 flex items-center justify-center font-bold text-white bg-gradient-to-br from-blue-400 to-blue-600">
                      {selectedChat.avatar ? (
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-bold dark:text-gray-100 truncate">{selectedChat.nombre}</p>
                  ...
                  {/* Messages Area */}
                  <div ref={messagesContainerRef} className={`flex-1 overflow-y-auto p-3 md:p-4 space-y-3 md:space-y-4 ${currentMode === 'Dark' ? 'bg-gray-900' : 'bg-gray-50'}`}>
                    {chatLoading ? (
                      ...
                      <div className="flex flex-col items-center justify-center h-full text-center">
                        <FaComments className="text-5xl md:text-6xl text-gray-300 dark:text-gray-600 mb-4" />
                          ...
                          return (
                            <React.Fragment key={msg._id || index}>
                              ...
                              <div className={`flex ${isOwn ? 'justify-end' : 'justify-start'}`}>
                                <div className={`max-w-[85%] md:max-w-[70%] ${isOwn ? 'order-2' : ''}`}>
                  ...
                  {/* Message Input */}
                  <div className={`p-3 md:p-4 border-t ${currentMode === 'Dark' ? 'border-gray-700 bg-gray-800' : 'border-gray-200 bg-white'}`}>
                    <div className="flex items-center gap-2 md:gap-3">
                      <input
                        ...
                        className={`flex-1 px-3 py-2.5 md:px-4 md:py-3 rounded-xl border focus:ring-2 focus:ring-blue-500 transition-all text-sm md:text-base ${
                          currentMode === 'Dark'
                      ...
                      <button
                        ...
                        className="p-2.5 md:p-3 rounded-xl bg-gradient-to-r from-blue-500 to-blue-600 text-white hover:from-blue-600 hover:to-blue-700 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex-shrink-0"
                      >
                        {sendingMessage ? (
