  ...
  const renderBreadcrumb = () => {
    ...
    return (
      <div style={S.breadcrumb}>
        <button onClick={() => navigateToFolder(null)} style={{ border: 'none', background: 'transparent', color: IOS.blue, cursor: 'pointer', fontSize: 13, fontFamily: 'inherit', padding: 0, flexShrink: 0 }}>Mis Archivos</button>
        {breadcrumb.map((b, i) => (
      ...
      <div style={S.overlay} onClick={() => setMoveTarget(null)}>
        <div style={{ ...S.sheet, maxHeight: '60vh' }} onClick={(e) => e.stopPropagation()}>
          ...
          <div style={{ padding: 8, maxHeight: 300, overflowY: 'auto' }}>
            {moveParent && (
              <button onClick={() => handleMoveBrowse(null)} style={{ ...S.sheetAction(false), fontSize: 15 }}>← Mis Archivos</button>
            )}
