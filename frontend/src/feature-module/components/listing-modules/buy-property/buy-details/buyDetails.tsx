...
const BuyDetails = () => {
  ...
  // Inline visit form state (preserved)
  ...
  // Sticky bar + scroll depth tracking (preserved)
  useEffect(() => {
  ...
  // ─── Funnel design tokens from funnelSettings ────────────────────────────
  ...
  // ─── Extra features for detailed specs table ─────────────────────────────
  ...
  const faqs = [
    { q: "¿Cuáles son los gastos de expensas y servicios?", a: "Podemos darte información detallada sobre los gastos regulares. Contactanos para recibir el detalle actualizado de expensas y servicios incluidos." },
    { q: "¿Acepta financiación o crédito hipotecario?", a: "Trabajamos con múltiples alternativas de financiación. Nuestro equipo puede orientarte sobre los mejores instrumentos disponibles según tu situación." },
    { q: "¿Cuál es el proceso para realizar una oferta?", a: "El proceso es simple y acompañado por nuestros asesores en cada paso. Coordina una visita y te explicamos todo en detalle." },
    { q: "¿Qué documentación se necesita?", a: "Variará según la operación. En general, se requiere DNI, constancia de ingresos y, en caso de compra, escritura del bien. Contáctanos y te asesoramos." },
  ];
  ...
  const operLabel = property.operation === "rent" ? "En Alquiler" : "En Venta";
  ...
  return (
    <>
        ...
        <div className="container pb-5">
          <div className="row gx-4 gx-xl-5">
            ...
            <div className="col-xl-8">
              ...
              {specs.length > 0 && (
                <div
                  className="mb-5 p-4 rounded-3 d-flex flex-wrap gap-3"
                  ...
                  <div className="row g-3">
                    {extraSpecs.map(([key, val]) => (
                      <div key={key} className="col-sm-6 col-md-4">
                        <div
                          className="p-3 rounded-3 d-flex flex-column"
                          style={{ background: "#f8fafc", border: "1px solid #e2e8f0", height: "100%" }}
                        >
                ...
                <div className="mb-5">
                  ...
                  <div className="d-flex flex-wrap gap-2">
                    {amenities.map((a) => (
                      <span
                        key={a}
                        className="badge fw-normal px-3 py-2"
                        style={{ background: `${accentColor}15`, color: accentColor, fontSize: "0.85rem", borderRadius: 50 }}
                      >
                        {a}
                      </span>
                    ))}
              ...
              {property.agent?.name && (
                <div className="mb-5">
                  ...
                  <div
                    ...
                    style={{ background: "linear-gradient(135deg,#f8fafc,#f0f4f8)", border: "1px solid #e2e8f0" }}
                  >
                      ...
                      <div className="col">
                        <div className="d-flex align-items-center gap-2 mb-1 flex-wrap">
                          ...
                          {property.agent.cargo && (
                            <span className="badge" style={{ background: `${accentColor}20`, color: accentColor, fontSize: "0.75rem" }}>
                              {property.agent.cargo}
                            </span>
                          )}
                        </div>
                        ...
                        <div className="d-flex flex-wrap gap-2">
                          {property.agent.phone && (
                            <a href={`tel:${property.agent.phone}`} className="btn btn-sm btn-outline-secondary d-flex align-items-center gap-1">
                              <i className="material-icons-outlined" style={{ fontSize: 16 }}>phone</i>
                              {property.agent.phone}
                            </a>
                          )}
                          {property.agent.email && (
                            <a href={`mailto:${property.agent.email}`} className="btn btn-sm btn-outline-secondary d-flex align-items-center gap-1">
                              <i className="material-icons-outlined" style={{ fontSize: 16 }}>email</i>
                              Email
                            </a>
                          )}
                          {whatsappNumber && (
              ...
              <div className="mb-5">
                ...
                <div className="d-flex flex-column gap-2">
                  {faqs.map((faq, i) => (
                    ...
                    >
                      <button
                        className="btn w-100 text-start d-flex align-items-center justify-content-between p-4 fw-semibold"
                        style={{ gap: 12, fontSize: "0.95rem" }}
                        onClick={() => setFaqOpen(faqOpen === i ? null : i)}
                      >
                        <span>{faq.q}</span>
                        <i className="material-icons-outlined text-secondary" style={{ fontSize: 20, transition: "transform 0.2s", transform: faqOpen === i ? "rotate(180deg)" : "rotate(0deg)" }}>
                          expand_more
                        </i>
                      </button>
                      {faqOpen === i && (
                        <div className="px-4 pb-4" style={{ fontSize: "0.9rem", color: "#4b5563", lineHeight: 1.7 }}>
                          {faq.a}
                        </div>
                      )}
                    </div>
                ...
                >
                  ...
                  <div className="d-flex flex-wrap gap-3 mb-4">
                    {property.visitCount && property.visitCount > 0 ? (
                      <span className="badge" style={{ background: "rgba(255,255,255,0.12)", color: "#fff", fontSize: "0.82rem", padding: "6px 14px" }}>
                        👁 {property.visitCount} personas vieron esta propiedad
                      </span>
                    ) : null}
                    {property.trending && (
                      <span className="badge" style={{ background: "rgba(239,68,68,0.25)", color: "#fca5a5", fontSize: "0.82rem", padding: "6px 14px" }}>
                  ...
                  <h4 className="fw-bold mb-1" style={{ fontSize: "1.4rem" }}>Solicitá tu visita privada</h4>
                  <p style={{ color: "rgba(255,255,255,0.6)", marginBottom: "1.5rem", fontSize: "0.92rem" }}>
                    Completá el formulario y un asesor te contactará a la brevedad para coordinar.
                  </p>

                  {visitFeedback && (
                    <div
                      className={`alert ${visitFeedback.type === "success" ? "alert-success" : "alert-danger"} mb-3`}
                  ...
                  <div className="d-flex flex-wrap gap-3 mt-4">
                    <button
                      type="button"
                      className="btn btn-lg fw-bold px-4 d-flex align-items-center gap-2"
                    ...
                    {whatsappNumber && (
                      <button
                        type="button"
                        className="btn btn-lg fw-bold px-4 btn-success d-flex align-items-center gap-2"
                        style={{ borderRadius: 12 }}
                        onClick={() => openWhatsApp("form")}
                      >
              ...
              {/* ── BLOCK: FINAL CTA ────────────────────────────────────────── */}
              <div
                className="mb-5 text-center p-5 rounded-4"
              ...
              >
                ...
                <h4 className="fw-bold mb-2">¿Te interesa esta propiedad?</h4>
                <p style={{ color: "rgba(255,255,255,0.7)", marginBottom: "1.5rem", fontSize: "0.95rem" }}>
                  Coordiná una visita y recibí asesoramiento personalizado para avanzar con seguridad.
                </p>
                <div className="d-flex justify-content-center flex-wrap gap-3">
                  <button
                    className="btn btn-light btn-lg px-4 fw-bold d-flex align-items-center gap-2"
                    onClick={() => { scrollToForm(); trackEvent("final_cta_click", { propertyId: property.id }); }}
                  >
                    <i className="material-icons-outlined">calendar_today</i> Agendar visita
                  </button>
                  {whatsappNumber && (
                    <button
                      className="btn btn-success btn-lg px-4 fw-bold d-flex align-items-center gap-2"
      ...
      >
        ...
        >
          <i className="material-icons-outlined" style={{ fontSize: 18 }}>calendar_today</i> Agendar visita
        </button>
        {whatsappNumber && (
          <button
            className="btn btn-success fw-semibold d-flex align-items-center gap-2"
            style={{ borderRadius: 10 }}
            onClick={() => openWhatsApp("mobile_cta")}
          >
            <i className="fa-brands fa-whatsapp" /> WhatsApp
          </button>
        )}
